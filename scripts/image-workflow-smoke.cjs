...
;(async () => {
  ...
  try {
    ...
    const waitSaved = () => page.waitForFunction(() => document.querySelector('.image-workflow-save-state')?.textContent === '切换时自动保存')
    ...
    await pasted.waitForFunction(() => document.querySelector('.image-workflow-save-state')?.textContent === '切换时自动保存')
    await pasted.reload()
