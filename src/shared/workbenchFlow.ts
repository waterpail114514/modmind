...
import { draftProjectContext, draftTargetFromMessage, missingDraftDetails } from './draftProject'
...
export function shouldAutoStartDraft(project: Pick<ProjectInfo, 'draft'>, message: string): boolean {
  if (!project.draft || missingDraftDetails(project).length) return false
  if (/介绍|讲解|解释|对比|比较|怎么|如何|为什么|能否|是否|什么|先聊|先讨论|暂不|先不|只讨论|只聊|先别/.test(message)) return false
  // Only a concrete user selection can finish setup; assistant recommendations cannot.
  return Object.keys(draftTargetFromMessage({}, message)).length > 0
}
...
export function engineeringHandoffPrompt(context: string, automatic = false): string {
  return `${automatic ? '用户已确认作品类型、版本和平台，应用已自动创建工程，现在开始制作。' : '用户已点击“开始制作”。'}请根据下面的完整对话实施用户认可的方案，继续使用当前项目，先检查已有实现。讨论中的建议不等于已完成的修改；不要把未选择的互斥方案全部实施。按工作台工程流程完成修改、构建和验证；仍缺少必要信息时先追问，不要猜测。\n\n${context}`
}
