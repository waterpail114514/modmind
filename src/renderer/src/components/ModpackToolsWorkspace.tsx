...
export default function ModpackToolsWorkspace({ project, section, onOpenModule }: { project: ProjectInfo; section: ToolSection; onOpenModule?: (module: ProjectInfo) => void }): React.JSX.Element {
  ...
  const renderContent = (): React.JSX.Element => <>
    ...
    <section className="modpack-tool-section">
      <div className="modpack-tool-heading"><BookOpen size={18} /><div><h2>Patchouli 指南书</h2></div></div>
  ...
  const renderAutomation = (): React.JSX.Element => <>
    ...
    <section className="modpack-tool-section">
      <div className="modpack-tool-heading"><PackageSearch size={18} /><div><h2>Mod 方案</h2><p>每行一个名称，或用逗号分隔</p></div></div>
    ...
    <section className="modpack-tool-section">
      <div className="modpack-tool-heading"><ShieldCheck size={18} /><div><h2>锁定审计</h2></div></div>
    ...
    <section className="modpack-tool-section">
      <div className="modpack-tool-heading"><Gauge size={18} /><div><h2>性能优化</h2></div></div>
        ...
        <details className="server-side-panel"><summary>运维与验证</summary>
          ...
          {project.kind === 'modpack' ? <><div className="server-operation-list">
            <button className="server-operation" disabled={serverBusy || serverState.running} onClick={() => runServerAction('pack', async () => {
              ...
              return result
            }, '服务端包已更新')}><FileCheck2 size={17} /><span><strong>同步服务端包</strong></span></button>
            <button className="server-operation" disabled={serverBusy || !runtimeReadyToCheck} title={runtimeReadyToCheck ? undefined : '请先同步服务端包'} onClick={() => runServerAction('runtime', () => window.modmind.modpack.installServerRuntime(serverInput), '服务端运行时已就绪')}><Server size={17} /><span><strong>检查运行时</strong></span></button>
  ...
  const renderServer = (): React.JSX.Element => <>
    ...
    <section className="modpack-tool-section">
      <div className="modpack-tool-heading"><Play size={18} /><div><h2>本机联机验证</h2></div></div>
